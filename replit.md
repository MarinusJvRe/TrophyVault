...
## Recent Changes
- **2026-03-21**: Beta launch readiness: cleanup, hardening & production fixes. Removed dead code: `mock-data.ts`, `placeholder.tsx`, `client/replit_integrations/audio/`, `server/replit_integrations/image/` (routes/index only, openai client moved inline to trophy-3d.ts), ~28 unused Shadcn UI components. Auth tokens now persisted to `auth_tokens` DB table (survives server restarts). 3D model generation state moved from in-memory `pendingModels` Map to DB-backed `modelStatus` column on trophies table. `fs.readFileSync` in analyze route replaced with async `fs.promises.readFile`. External API timeouts added: 60s for OpenAI AI analysis, 120s for fal.ai/Meshy/GPT-image-1 3D generation calls. 3D generation failures now show toast notification to user. Referral tracking endpoint hardened with user existence validation and duplicate checks. Database schema synced.
