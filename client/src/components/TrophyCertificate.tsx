...
export async function generateTrophyCertificate(
  ...
  doc.setTextColor(...MUTED);
  doc.text(
    `Generated by TrophyVault on ${new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}`,
    pageWidth / 2,
    footerY - 2,
    { align: "center" }
  );
  doc.text("trophyvault.app", pageWidth / 2, footerY + 2, { align: "center" });

  const filename = `TrophyVault_${trophy.species.replace(/\s+/g, "_")}_${trophy.date || "certificate"}.pdf`;
  doc.save(filename);
}
