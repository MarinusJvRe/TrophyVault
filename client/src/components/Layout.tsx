import { Trophy, Crosshair, Map, Calendar, User, Settings, LogOut, PlusCircle } from "lucide-react";
  ...
  return (
    <div className="flex h-screen w-full bg-background text-foreground overflow-hidden">
      ...
      <aside className="w-64 border-r border-border/40 bg-card/50 backdrop-blur-xl hidden md:flex flex-col">
        ...
        <div className="p-4 border-t border-border/40">
           ...
           <div className="flex gap-2 mt-2">
             <button className="flex-1 flex items-center justify-center gap-2 py-2 text-xs font-medium text-muted-foreground hover:text-foreground hover:bg-white/5 rounded-md transition-colors">
      ...
      <main className="flex-1 overflow-auto relative">
        <div className="absolute inset-0 z-0 pointer-events-none bg-[url('/noise.png')] opacity-[0.03] mix-blend-overlay"></div>
        {children}
      </main>
    </div>
  );
